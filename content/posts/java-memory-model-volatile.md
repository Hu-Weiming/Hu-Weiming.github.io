...
Java 编译器和处理器为了优化性能，可能会对指令进行重排序。单线程下重排序不影响结果（as-if-serial），但多线程下就可能出问题。
...
线程 A 里语句 1 和语句 2 可能被重排，先执行 `flag = true`，再执行 `a = 1`。这时线程 B 看到 flag 为 true，但 a 还是 0。
...
happens-before 这个概念比较抽象，但搞懂它对理解并发很重要。面试的时候也喜欢问。
...
`instance = new Singleton()` 这行代码，在字节码层面大概是三步：
1. 分配内存空间
2. 调用构造方法初始化对象
3. 把引用赋值给 instance

如果不加 volatile，步骤 2 和 3 可能被重排成 3、2。这时候线程 B 在第一个 if 判断时看到 instance 不为 null，直接返回了一个还没初始化完的对象。用了就 NPE 或者各种奇怪问题。

加了 volatile 之后，禁止了这个重排序，保证赋值一定在初始化之后。

这个坑是 Java 5 之前的 volatile 语义不够强导致的。Java 5 增强了 volatile 的语义（JSR-133），DCL 才真正安全了。

## volatile 不能保证原子性

很多人以为 volatile 能保证线程安全，这是个误区。volatile 只保证可见性和有序性，不保证原子性。
...
`count++` 实际上是读-改-写三步操作。就算 volatile 保证了每次读到最新值，两个线程还是可能同时读到同一个值，然后各自加1写回，相当于少加了一次。

需要原子性的话，用 `AtomicInteger` 或者加锁。
...
适用场景：状态标志（如上面的 flag 例子）、DCL 中防止重排序、一写多读的场景。如果是多写场景，还是得用锁或原子类。
